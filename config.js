...
hfc.setConfigSetting('network-connection-profile', path.join(__dirname, 'network', file));

organizations.forEach((organization) => {
	// indicate to the application where the setup file is located so it able
	// to have the hfc load it to initalize the fabric client instance
	hfc.setConfigSetting(organization, path.join(__dirname, 'network/wallet-conn-profile', organization.toLowerCase() + '.yaml'));
});
